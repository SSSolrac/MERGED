...
export const DashboardLayout = () => {
  ...
  const navItems = useMemo(() => {
    const base = [
      { section: 'Operations', label: 'Dashboard Overview', path: '/dashboard', icon: LayoutDashboard },
      { section: 'Operations', label: 'View Orders', path: '/orders', icon: ClipboardList },
      { section: 'Catalog', label: 'Edit Daily Menu', path: '/daily-menu', icon: CalendarDays },
      { section: 'Catalog', label: 'Manage Menu Items', path: '/menu', icon: Utensils },
      { section: 'Catalog', label: 'Inventory', path: '/inventory', icon: Boxes },
  ...
  return (
    <div className="min-h-screen bg-[#FFF7F9] text-[#1F2937]">
      <div className="flex">
        <aside className="hidden md:flex md:sticky md:top-0 md:self-start w-64 h-screen border-r bg-white flex-col py-3 px-3">
          <div className="h-10 w-10 rounded-full bg-[#FF8FA3] text-white flex items-center justify-center font-semibold text-sm mx-2">
            SO
          </div>
          <nav className="mt-4 flex-1 w-full space-y-3 overflow-y-auto">
            {navSections.map(([section, sectionItems]) => (
              <div key={section} className="space-y-1">
                <p className="px-2 text-[11px] uppercase tracking-wide text-slate-400">{section}</p>
          ...
          <button
            onClick={onSignOut}
            title="Sign out"
            className="mt-auto mb-1 w-full flex items-center gap-3 rounded-lg px-3 py-2 text-slate-600 hover:bg-[#FFF3F5] transition-colors"
          >
        ...
        <section className="flex-1">
          <header className="h-16 bg-[#FF8FA3] text-white px-4 md:px-5 flex items-center justify-between">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-white/20 ring-1 ring-white/35 shadow-sm overflow-hidden">
                  <img src={happyTailsLogo} alt="Happy Tails Pet Cafe" className="h-8 w-8 object-contain" />
                </div>
        ...
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-72 bg-white shadow-xl p-4 flex flex-col">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 min-w-0">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#FFF3F5] ring-1 ring-[#F6D2DA] shadow-sm overflow-hidden">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500">Staffowner</p>
                </div>
              </div>
