...
const isDashboardConfirmedPayment = (order: Pick<Order, 'paymentStatus'>) => {
  const normalizedPaymentStatus = normalizeStatusValue(order.paymentStatus);
  return normalizedPaymentStatus === 'paid' || normalizedPaymentStatus === 'refunded';
};
...
export const dashboardService = {
  ...
  async getDashboardData(range: DateRangePreset, options: { includeFinancialSummary?: boolean } = {}): Promise<DashboardData> {
    ...
    const confirmedLiveOrders = liveOrdersWithRelations.filter(isDashboardConfirmedPayment);
    const sortedLiveOrders = [...confirmedLiveOrders].sort((a, b) => orderTimestampMs(b) - orderTimestampMs(a));
    ...
    const resolved = {
      ...
      topItems: buildTopItemsFromOrders(combinedOrders),
